...
def velthuis(devanagari):
    ...
    text = re.sub('(%s)(%s)' % (re_consonant, re_vowel_sign),
                  lambda match: consonants[ord(match.group(
                      1))][0] + vowel_signs[ord(match.group(2))][0],
                  text)
...
def wikner(devanagari):
    ...
    text = re.sub('(%s)(%s)' % (re_consonant, re_vowel_sign),
                  lambda match: consonants[ord(match.group(
                      1))][-1] + vowel_signs[ord(match.group(2))][-1],
                  text)
    text = re.sub('(%s)(%s)' % (re_consonant, re_virama),
...
if __name__ == '__main__':
    ...
    for text in file_data.split('\n'):
        if len(text) != 0:
            ...
            with open('dn_results.devnagout.txt', 'a', encoding='utf8') as outFile:
                outFile.write('{\dn ' + out + '}\n')
